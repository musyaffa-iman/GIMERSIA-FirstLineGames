...
@export var attack_damage: int = 40 # ATK value (kept for compatibility with any melee fallback)
...
func explode() -> void:
    ...
    for r in results:
        ...
        if collider and collider.has_method("take_damage"):
            ...
            var ndir = dir.length() > 0 ? dir.normalized() : Vector2.ZERO
...
func take_damage(amount: int, from_direction: Vector2 = Vector2.ZERO, knockback_force: float = 300.0) -> void:
    ...
    var will_die = (max_health - amount) <= 0
